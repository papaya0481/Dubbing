...
#
# Copied from indextts/s2mel/modules/gpt_fast/model.py with no modifications
# to preserve weight compatibility.
...
class Transformer(nn.Module):
    ...
    def setup_caches(self, max_batch_size: int, max_seq_length: int, use_kv_cache: bool = True):
        ...
        self.freqs_cis = precompute_freqs_cis(
            self.config.block_size, self.config.head_dim,
        ...
        if self.uvit_skip_connection:
            ...
            self.layers_emit_skip = [i for i in range(n) if i < n // 2]
            self.layers_receive_skip = [i for i in range(n) if i > n // 2]
        else:
    ...
    def forward(
        ...
        if self.has_cross_attention:
            h = h + self.cross_attention(
                self.cross_attention_norm(h, c), freqs_cis,
                cross_attention_mask, input_pos, context, context_freqs_cis,
            )
