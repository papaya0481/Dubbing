...
def _is_decimal_point_in_text(text: str, idx: int) -> bool:
    """Check whether text[idx] is a decimal point between digits, e.g. 3.14."""
...
class Text2SRT():
    ...
    def __init__(self, text, timestamp, offset=0, normalize=True):
    ...
    def text(self):
        if isinstance(self.token_list, str):
            ...
            if (self.TERMINAL_PUNCTUATION_ZH.search(text) or 
                ...
                return self._apply_normalization(text)
            # 以其他标点结尾（如 , ; : ， ； ： 、）→ 替换为句号
            if self.NON_TERMINAL_PUNCTUATION.search(text):
                is_zh = self._is_chinese_context(text)
                result = re.sub(self.NON_TERMINAL_PUNCTUATION, '。' if is_zh else '.', text)
                return self._apply_normalization(result)
            # 无任何标点结尾 → 添加合适句号
            is_zh = self._is_chinese_context(text)
            result = text + ('。' if is_zh else '.')
            return self._apply_normalization(result)
        else:
            res = ""
            ...
            if (self.TERMINAL_PUNCTUATION_ZH.search(res) or 
                ...
                return self._apply_normalization(res)
            if self.NON_TERMINAL_PUNCTUATION.search(res):
                is_zh = self._is_chinese_context(res)
                result = re.sub(self.NON_TERMINAL_PUNCTUATION, '。' if is_zh else '.', res)
                return self._apply_normalization(result)
            is_zh = self._is_chinese_context(res)
            result = res + ('。' if is_zh else '.')
            return self._apply_normalization(result)
...
def normalize_sentence_info(sentence_list):
    """
    对 sentence_info 中的每个 sentence 的 text 进行正规化。
    保持 timestamp 不变，确保 text 与 timestamp 配对。
    跳过包含英文缩写的文本（如 here's, don't, it's）。
    打印被改变的文本。
    
    Args:
        sentence_list: 句子信息列表，每个元素格式为 {'text': ..., 'timestamp': ..., ...}
        
    Returns:
        正规化后的 sentence_list（原地修改）
    """
    if not sentence_list:
        return sentence_list
    
    normalized_count = 0
    skipped_abbrev_count = 0
    skipped_mismatch_count = 0
    for sent in sentence_list:
        if 'text' in sent and isinstance(sent['text'], str):
            ...
            if ts:
                ...
                if len(normalized_tokens) != len(ts):
                    # Keep original text to avoid text/timestamp misalignment.
                    skipped_mismatch_count += 1
                    continue

            if original_text != normalized_text:
                normalized_count += 1
                print(f"[正规化] {normalized_count}. '{original_text}' → '{normalized_text}'")
            elif original_text == normalized_text and re.search(r"\w+'[a-z]", original_text, re.IGNORECASE):
                # 文本没变，但检测到缩写，说明被跳过了
                skipped_abbrev_count += 1
            sent['text'] = normalized_text
    
    if normalized_count > 0:
        print(
            f"[正规化总结] 共正规化 {normalized_count} 个句子，"
            f"跳过 {skipped_abbrev_count} 个含缩写句子，"
            f"跳过 {skipped_mismatch_count} 个词数不匹配句子\n"
        )
    elif skipped_abbrev_count > 0 or skipped_mismatch_count > 0:
        print(
            f"[正规化总结] 跳过 {skipped_abbrev_count} 个含缩写句子，"
            f"跳过 {skipped_mismatch_count} 个词数不匹配句子\n"
        )
...
def generate_srt(sentence_list, normalize=False):
    srt_total = ''
    for i, sent in enumerate(sentence_list):
        t2s = Text2SRT(sent['text'], sent['timestamp'], normalize=normalize)
        if 'spk' in sent:
...
def generate_srt_clip(sentence_list, start, end, begin_index=0, time_acc_ost=0.0, normalize=False):
    ...
    for _, sent in enumerate(sentence_list):
        ...
        if (sent['timestamp'][-1][1] <= end and sent['timestamp'][0][0] > start) or (sent['timestamp'][-1][1] == end and sent['timestamp'][0][0] == start):
            t2s = Text2SRT(sent['text'], sent['timestamp'], offset=start, normalize=normalize)
            if 'spk' in sent:
        ...
        if sent['timestamp'][0][0] <= start:
            ...
            if _ts and len(_ts) > 0:
                t2s = Text2SRT(_text, _ts, offset=start, normalize=normalize)
                if 'spk' in sent:
                    srt_total += "{}  spk{}\n{}".format(cc, sent['spk'], t2s.srt(time_acc_ost))
                else:
                    srt_total += "{}\n{}".format(cc, t2s.srt(time_acc_ost))
...
def process_asr_to_sentence_info(rec_result):
    ...
    for i, char in enumerate(full_text):
        ...
        # Do not split on decimal points inside numbers, e.g. "3.14".
        if char == '.' and _is_decimal_point_in_text(full_text, i):
            continue
