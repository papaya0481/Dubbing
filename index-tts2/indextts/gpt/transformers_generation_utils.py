...
class GenerateBeamDecoderOnlyOutput(ModelOutput):
    ...
    hidden_states: Optional[Tuple[Tuple[torch.FloatTensor]]] = None
    past_key_values: Optional[Tuple[Tuple[Tuple[torch.FloatTensor]]]] = None
    attention_mask_ids: Optional[List[List[int]]] = None  # 每个token对应的attention mask id
...
class GenerationMixin:
    ...
    def heal_tokens(
        ...
        # 搞一堆mask，显存有可能爆，如果开了内存共享，torch不会自动清显存，而是会去用内存，搞得特别慢，这里释放下显存
    ...
    def _beam_search(
        ...
        **model_kwargs,
        ...
        model_kwargs["attention_phase"] = [0] * (batch_size * num_beams) # 记录情感阶段，用于选择不同的attention mask，从0开始到len(attention_masks)-1
        emotion_final_phase = len(attention_masks) - 1
        tokenwise_attention_mask = [list() for _ in range(batch_size * num_beams)]  # 记录每个token对应的attention mask id
        ...
        while self._has_unfinished_sequences(this_peer_finished, synced_gpus, device=input_ids.device):
            ...
            # 2. 重排 segment_positions
            # segment_positions 是一个 dict {beam_id: [pos_list]}
            ...
            cur_len = cur_len + 1
            if cur_len >= 1500:
                raise ValueError("生成长度超过1000，强制停止，可能是模型未学习到结束符eos_token_id的使用，请检查数据和训练过程。")
        ...
        return GenerateBeamDecoderOnlyOutput(
            ...
            hidden_states=decoder_hidden_states,
            past_key_values=model_kwargs.get("past_key_values"),
            attention_mask_ids=tokenwise_attention_mask_ids,  # 每个token对应的attention mask id
        )
