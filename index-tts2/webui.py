...
def gen_single(emo_control_method,prompt, text,
    ...
    do_sample, top_p, top_k, temperature, \
        length_penalty, num_beams, repetition_penalty, max_mel_tokens, input_full_attention_mask, method = args
    kwargs = {
        ...
        "input_full_attention_mask": bool(input_full_attention_mask),
        "method": str(method),
        ...
        with gr.Accordion(i18n("高级生成参数设置"), open=False):
            with gr.Row():
                with gr.Column(scale=1):
                    ...
                    input_full_attention_mask = gr.Checkbox(label="input_full_attention_mask", value=False, info=i18n("使用完整注意力掩码（实验性功能）"))
                    method = gr.Dropdown(label="method", choices=["eos", "attention_weight", "attention_fs", "attention_fs_topk", "hmm"], value="hmm", info=i18n("用于情绪转换的对齐方式"))
                    # with gr.Row():
            ...
            advanced_params = [
                do_sample, top_p, top_k, temperature,
                length_penalty, num_beams, repetition_penalty, max_mel_tokens, input_full_attention_mask, method,
